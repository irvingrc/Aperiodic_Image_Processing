...
def segmentize(image):
    ...
    return nodes.astype(bool), seg.astype(bool), sk.astype(bool)
...
def connectivity(segments, nodes, dil_nodes, return_map=False):
    '''
    ...
    return node_connectivity, connect_map, c
...
def thickness(im, nodes, sk):
    ...
    return dist_edt, strut_thick, node_thick
...
def color_by(labelled, reg, feat='area', func=None, amin=0):
    ...
    if func is not None:
        ...
        for r in range(len(reg)):
            coords, f = reg.loc[r, 'coords'], reg.loc[r, feat]
            colored[tuple(zip(*coords))] = f
            feat_list.append(f)        
